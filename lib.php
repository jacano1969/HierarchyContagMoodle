...
function contag_get_all_tags_with_their_items($course_id) {
	...
	foreach (contag_get_items_for_course($course_id) as $item) {
		contag_generate_display_properties_for_item($item);
		// SIDE EFFECTS - $item is modified
	...
	foreach ($defined_tags as $tag) {
		...
		foreach ($assocs as $assoc) {// for each tagging instance
			...
			if (!isset($all_items[$the_key])) {// stale item, delete it from custom table AND association (i.e. cleanup as we go)
				...
				$item = $all_items[$the_key];
				// grab the cached item that this assoc refers to
				$item -> tagged = true;
				// so we don't include it in our untagged set
				$tag -> items[] = $item;
			}
		}
	}
	...
	// untagged items
	$tag = new StdClass;
	$tag -> tag_name = $CONTAG_NO_TAG_KEY;
	// NOTE: does not have course_id/fake tag_id
	$tag -> items = array();
	...
	foreach (contag_get_inbuilt_item_types() as $intype) {
		...
		foreach ($records as $item) {
			if (!get_coursemodule_from_id($intype, $item -> item_id)) {
				...
				$DB -> delete_records('block_contag_association', array('item_type' => $intype, 'item_id' => $item -> id));
				// delete associations first
				$DB -> delete_records(contag_table_from_item_type($intype), array('id' => $item -> id));
				// delete item itself
			}
		}
	}
