    ...
    function get_content() {
        ...
        if (has_capability('block/contag:view', $context)) { // can they view it?
            ...
            if (has_capability('block/contag:edit', $context)){
            	$res .= '<br/><li>'.'<a href="'.$link.'edit_tags.php?id='.$courseid.'">'.get_string('edit_hierarchy_tree', 'block_contag').'</a>'.'</li>';
                $res .= '<br/><li>'.'<a href="'.$link.'attach_tags.php?id='.$courseid.'">'.get_string('edit_concept_tags', 'block_contag').'</a>'.'</li>';
