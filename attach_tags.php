...
$url = new moodle_url('/blocks/contag/attach_tags.php', array('id'=>$courseid));
$PAGE->set_url($url);
...
if (has_capability('block/contag:edit', $context)){ // can they edit tags?
    ...
    var tag_datasource = new YAHOO.util.LocalDataSource(contag_tags);
    ...
    function custom_item_del_link_if_necessary($item, $courseid){
        ...
        if (contag_is_custom_item_type($item->type)){
            return '<a href="attach_tags.php?id='.$courseid.'&'.$CONTAG_DELETE_CUSTOM_ITEM_KEY_NAME.'=TRUE&item_key='.$item->unique_key.'"><sup>[x]</sup></a>';
        } else {return '';}
    }
    ...
    foreach($tags_by_item as $item){
        ...
        foreach($item->tags as $tag){
            $tagurls[] = contag_get_tag_name($tag).'<a href="attach_tags.php?id='.$courseid.'&'.$CONTAG_UNASSOCIATE_KEY_NAME.'=TRUE&item_key='.$item->unique_key.'&tag_id='.$tag->id.'" title="Remove tag from \''.contag_get_item_display_name($item).'\'" ><sup>[x]</sup></a>';
        }
        ...
        $table->data[] = array(contag_get_html_link_for_item($item).custom_item_del_link_if_necessary($item, $courseid), $item->type, implode(", ", $tagurls), 
                               '<div class="myAutoComplete"><input id="'.$input_id.'" type="text" name="'.$CONTAG_TAG_FIELD_NAME.'_'.$item->unique_key.'" autocomplete=off tabindex='.$tabindex++.'/> <span id="'.$container_id.'"></span></div><script type="text/javascript"> createNewAutocompleteWidget("'.$input_id.'","'.$container_id.'");</script>'); // add row - second column combines all tags with ", "
    ...
    print('<form name="addtagform" method="post" action="attach_tags.php?id='.$courseid.'" onSubmit="submitting=true;return true;">');
    ...
    function format_deltag_link($tag){
        ...
        return '<span class="editable" style="cursor: pointer;'.$colour_string.'" onmouseover="this.style.backgroundColor=\'#DDDDDD\';" onmouseout="this.style.backgroundColor=\'white\';" title="Click to rename">'.$tagname.'</span>'.'<a href="attach_tags.php?id='.$courseid.'&'.$CONTAG_DELETE_TAG_KEY_NAME.'=TRUE&tag_id='.$tag->id.'" onclick="return confirm(\'Are you sure you want to permanently delete this tag?\');" title="Permanently delete tag"><sup>[x]</sup></a>';
    }
   
	//the functionallity below has been removed
    ...
    (function() {
        var Dom = YAHOO.util.Dom,
            Event = YAHOO.util.Event;
        editable.callback = function(){var id = getIDForOriginal(editable.contents); window.location="attach_tags.php?id="+<?php echo $courseid ?>+"&<?php echo $CONTAG_RENAME_TAG_KEY_NAME ?>=TRUE&tag_id="+id+"&tag_new_name="+escape(editable.contents_new)};
        Event.onAvailable('editarea', editable.init, editable, true);
          ...
          /*
           ...
           <form method="post" action="attach_tags.php?id=<?php echo $courseid ?>">
